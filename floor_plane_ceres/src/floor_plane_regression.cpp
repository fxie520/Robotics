...
struct PlaneError {
    ...
    template <typename T>
        bool operator()(const T* const w,  // w is the array of parameters to solve
                T* residuals) const {
            // TODO START
            ...
            residuals[0] = weight*(z - w[0]*x - w[1]*y - w[2]);
...
class FloorPlaneRegression {
    ...
    protected: // ROS Callbacks
        void pc_callback(const sensor_msgs::PointCloud2ConstPtr msg) {
            ...
            // The parameter of the optimisation will be stored in the following buffer:
            double X[3] = {0,0,0};
            for (unsigned int i=0;i<n;i++) {
                ...
                loss_function = FLAGS_robustify ? new ceres::HuberLoss(1.0) : NULL;  // condition ? result_if_true : result_if_false
                // TODO START
                // Use the PlaneError defined above to build an error term for the ceres optimiser (see documentation link above)
                cost_function = new ceres::AutoDiffCostFunction<PlaneError, 1, 3>(new PlaneError(P.x, P.y, P.z, 1));
                // END OF TODO
                // This cost function is then added to the optimisation problem, with X as a parameter
    ...
    public:
        FloorPlaneRegression() : nh_("~") {
            // TODO: update these variable in the launch file, similarly to the linear regression case
