...
class PNGReader:
    ...
    def load_data(self):  # Data size: (nb of images*32*32*3)
        # LOAD TRAIN
        x_path, self.y_train = self.read_file(self.train_file)  # Read label file
    ...
    def load_x(self, path_list, root):
        ...
        for path in path_list:
            ...
            sx, sy = raw.shape[0:2]  # Get nb of pixels in x and y directions
            x.append(cv2.resize(raw,(0,0),fx=32.0/sx,fy=32.0/sy, interpolation = cv2.INTER_AREA))  # Resize image to 32*32
        return np.array(x)

    def compute_mean(self):
    	# Compute average of each channel, output: 1d array of 3 elements
        self.mean = np.mean(np.mean(np.mean(self.x_train, axis = 0), axis = 0),axis = 0)  
        # Compute std of each channel, output: 1d array of 3 elements
