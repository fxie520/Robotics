import os
os.environ['PYTHONHASHSEED']=str(2)  # This must be put before importing tf
...
class Train:
    def __init__(self):
        ...
        self.model = CNNModel3(32, 32, 3, self.settings.num_class, self.dataset.mean, self.dataset.std, self.settings.dropout)
    ...
    # The following decorator converts regular python code to a callable tf graph fct, which is usually more performant and python independent.
    @tf.function  
    ...
    def train(self):
        ...
        while (step < self.settings.max_iter):
            ...
            if step%25==0:
                ...
                x,y = self.sampler.sample_val_batch(1500)
                self.val_step(x,y) 
            if step%100==0:
                print('Step       : ', step)
