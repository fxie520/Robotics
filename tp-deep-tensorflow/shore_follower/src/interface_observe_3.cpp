...
class InterfaceObserve3 {
    ...
    protected: // ROS Callbacks
        ...
        void image_callback(const sensor_msgs::ImageConstPtr & img_msg) {
            ...
            if ((hypot(last_pose_.x-tfw.getOrigin().x(),last_pose_.y-tfw.getOrigin().y())<min_displacement_) &&
                    (fabs(remainder(last_pose_.theta-tf::getYaw(tfw.getRotation()),2*M_PI))<min_rotation_)) {  // Yaw angle in radian
                return;
            }
            // ROS_INFO("Displacement: %f", hypot(last_pose_.x-tfw.getOrigin().x(),last_pose_.y-tfw.getOrigin().y()));
			// ROS_INFO("Rotation: %f", fabs(remainder(last_pose_.theta-tf::getYaw(tfw.getRotation()),2*M_PI)));
            ...
            bool save_it = true;
            int label = 1;  // Went straight
            // The if block below is changed
            if (last_command_.angular.z<-rotation_threshold_) {
                label = 2;  // Turned right
    ...
    public:
        InterfaceObserve3() : nh_("~"), it_(nh_) {
            ...
            last_pose_.x=tfw.getOrigin().x()-0.0;  // -1.0 (for example) if the first image is to be saved
            last_pose_.y=tfw.getOrigin().y()-0.0;
            last_pose_.theta=tf::getYaw(tfw.getRotation());

            twist_sub_ = nh_.subscribe("twist",1,&InterfaceObserve3::twist_callback,this);
            joy_sub_ = nh_.subscribe("joy",1,&InterfaceObserve3::joy_callback,this);
            image_sub_ = it_.subscribe<InterfaceObserve3>("image",1, &InterfaceObserve3::image_callback,this,transport);
        }
};

int main(int argc, char * argv[]) 
{
    ros::init(argc,argv,"interface_observe_3");
    InterfaceObserve3 fp;
